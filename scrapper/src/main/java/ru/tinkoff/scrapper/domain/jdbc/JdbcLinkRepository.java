...
public class JdbcLinkRepository {
    ...
    public Link add(Link link) {
        return jdbcTemplate
            .queryForObject(
                "insert into links(link, last_update, state) "
    ...
    public int remove(Chat chat, Link link) {
        return jdbcTemplate.update(
            "delete from chats_to_links where link_id = :linkId and chat_id = :chatId",
    ...
    public List<Link> findOlderThan(int minutes) {
        return jdbcTemplate.query(
            "select *, now() - last_update from links where (now() - last_update) > (:interval * interval '1 minute')",
            new MapSqlParameterSource()
    ...
    public int addLinkToChat(Chat chat, Link link) {
        return jdbcTemplate.update(
            "insert into chats_to_links(chat_id, link_id)"
    ...
    public List<Chat> getChats(Link link) {
        return jdbcTemplate.query(
            "select * from chats join chats_to_links ctl on chats.id = ctl.chat_id where link_id = :linkId",
            new MapSqlParameterSource().addValue("linkId", link.getId()),
